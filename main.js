...
var hOne = function (input) {
  $("h1").text(input);
};
var hTwo = function (input) {
  $("h2").text(input);
};

$(document).ready(function () {
  hTwo("Welcome to what may be either your favorite dream or worst nightmare: The Zombie Apocalypse.");

  //switches from next button to radio buttons to choose roles.
  $('button').on('click', function () {
    ...
    $('button').on('click', function () {
      role = $('input[name=character]:checked').val();
      $('.char').hide();
      hTwo("Yeah, I thought you looked like a " + role + " type.  Good luck!");
        ...
        $('button').on('click', function () {
          ...
          if (choice === "run") {
            if (character.speed >= 4) {
              hTwo("You make a break for it, and all that running from the cops finally pays off.  Big Ugly can't keep up and you continue on your way as soon as you feel safe.  ");
            } else {
              hTwo("You make a break for it, but Big Ugly is faster.  You always told your kids that the only thing worth running for was free food.  He must have heard that in his past life.  He might not know who you are, but he's closing in and you've got to do something fast.  As a last resort you pull out your" + character.weapon + " And let him have it.  He drops dead of course, and you continue on with your business.");
            }
          } else if (choice === "fight") {
            if (character.weaponStrength >= 5) {
              hTwo("You pull out your " + character.weapon + " and let him have it.  He drops deader than he was before, of course, and you continue on with your business.");
            } else {
              hTwo("You pull out your" + character.weapon + " and let him have it.  He makes a sound as if he were laughing as he decends upon your face with a greedy grin.");
              notDead = false;
            }
          }
          ...
          $('button').on('click', function () {
            if (notDead) {
              hTwo("Bored, and a little irritated that your fun got ruined by Big Ugly, you decide to check on the group of travelers that you've been watching for a couple of days.  They are set up on the opposite side of town, and you aren't sure what to think of them yet.");
              $('button').on('click', function () {
                hTwo("You arrive to your lookout building and climb to the top.  As you look out over their camp, you are suprised at how quiet things are.  You see two or three people moving from car to car, but the rest are missing.  This worries you, but you decide its a good time to go check them out in person, and head back down the stairs. As you turn the corner to the last flight of stairs, you are greeted by, you guessed it, an entire platoon of zombies.  This day couldn't get any worse.  What will you do?  You can either fight it out, or run to the top of the building where you might be able to yell for help to the strangers camped out nearby.");
                $('.choices').show();
                $('button').on('click', function () {
                  $('.choices').hide();
                  choice = $('input[name=choices]:checked').val();
